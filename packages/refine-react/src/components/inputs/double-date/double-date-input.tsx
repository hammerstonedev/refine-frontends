...
export const DoubleDateInput = () => {
  const { display, date1, date2, onChange } = useInput<{ date1: string; date2: string }>();
  ...
  return (
    <div>
      ...
      <FlavorItem<'inputs.date.double.wrapper'> name="inputs.date.double.wrapper">
        ...
        />
        <FlavorItem<'inputs.date.double.joiner'> name="inputs.date.double.joiner">and</FlavorItem>
        <FlavorItem<'inputs.date.double'>
          name="inputs.date.double"
          value={date2}
